...
void ReadSVInfo(string file, map<Junction, Info> &junction2info, map<pair<string, int>, int> &start2end)
{
	...
	while (fin >> type)
	{
		if (type == "inv" || type == "INV")
		{
			...
			if (is_overlap == 0)
			{
				Junction junction("chr10", start_pos - 1, '+', "chr10", end_pos, '-');
				junction2info.insert(make_pair(junction, info));
				junction.set_value("chr10", start_pos, '-', "chr10", end_pos + 1, '+');
				junction2info.insert(make_pair(junction, info));
			}
		}
		getline(fin, temp);
	...
	while (fin >> type)
	{
		if (type == "lins")
		{
			...
			while (getline(strm, al_pos, ';'))
			{
				...
				if (is_overlap == 0)
				{
					Junction junction("chr10", insert_pos - 1, '+', "chr10", start_pos, '+');
					junction2info.insert(make_pair(junction, info_ins));
					junction.set_value("chr10", end_pos, '+', "chr10", insert_pos, '+');
					junction2info.insert(make_pair(junction, info_ins));
				}
			}
		}
		else if (type == "ldel")
		{
			...
			if (is_overlap == 0)
			{
				Junction junction("chr10", start_pos - 1, '+', "chr10", end_pos + 1, '+');
